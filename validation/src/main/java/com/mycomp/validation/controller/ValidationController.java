...
public class ValidationController {
	...
	public PaymentResponse createPayment(@RequestBody CreatePaymentReq createPaymentReq) {
		...
		log.info("create payment response: {}", createPaymentresponse);
		...
		intiatePaymentRequest.setCancelUrl("https://example.com/cancelUrl");
		intiatePaymentRequest.setSuccessUrl("https://example.com/returnUrl");
		...
		log.info("Payment initiated successfully for txnReference: {}", txnReference);
		log.info("Initiate payment response from validation service: {}", initiatePaymentResponse);
		return initiatePaymentResponse;
	}
	
	
	@PostMapping("/{txnReference}/complete-payment")
	public PaymentResponse completePayment(@PathVariable String txnReference) {
		log.info("Completing payment in validation service for the  txnResponse:{}", txnReference);
		
		PaymentResponse  response = paymentService.completePayment(txnReference);
		log.info("Complete payment response from service: {}", response);
		return response;
